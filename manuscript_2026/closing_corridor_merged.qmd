...
running out of runway. The runway's endpoint proves the stubborn term,
and for its near-fixity across three decades of postponement we will
propose, beyond stopping behaviour, a life-history interpretation - the
late reproductive years reallocated from childbearing to grandmothering -
...
- **Age and amount of peak fertility (AAPF).** The ridge of the surface in
  ...
  choice consequential; Supplement S8). The raw argmax is preferred to a
  smoothed estimate of the modal age deliberately: smoothing would
  introduce bandwidth choices that could not be fixed ex ante, and would
  blur exactly the plateau schedules of interest. The cost is volatility -
  on a near-flat schedule, sampling noise can shunt the strict maximum
  across a band of several ages between adjacent years, and the tie rule
  dampens but does not remove such steps - so AAPF trajectories are read
  for trend rather than year-to-year movement.
...
"speculative extrapolation of replacement contour": for Norway holding
near age 43 for a further decade of cohorts before turning vertical; for
the United States continuing near-flat at age 37. The drawn
...
re-rendered from the tagged data with the extrapolations restored, beside
the updated panels. On these composites a replacement contour can end in
two ways, and the distinction carries the results: a contour that escapes
vertically through the top of a fully observed region marks cohorts that
never reach the milestone at any age, while a contour cut at the diagonal
data edge is merely censored. @tbl-scoreboard states the resolutions.

![The 2020 extrapolations and what the new data showed. Birth cohorts run along the horizontal axis and age up the vertical, so period effects appear as 45-degree diagonals (Section 2.1). Rows: Norway, United States. Left column: the 2020-vintage data (repository tag `demres-2020`), re-rendered in the current visual specification, with the 2020-era speculative extrapolations in magenta, digitized from the September 2018 working figure (Supplement S7); they appear on the 2020-vintage panels only. Right column: the 2026 update. Heavy contour: CPCFR = 2.05 (replacement); thin: 1.50. Contours escaping through the top mark cohorts that never reach the milestone; contours cut at the data edge are censored.](figures/forecast_check_2x2.png){#fig-2x2}
...
- and then escapes vertically a second time immediately after the 1971
cohort. The drawn 2020 forecast had anticipated the same shape but a
decade later: its dotted line holds near age 43 through cohorts born to
about 1981 before turning vertical (@fig-2x2, left). The realized exit
came ten cohorts sooner, and was total.
...
reproduction, postponement, and policy variation. What movement there is
at the right tail, moreover, is increasingly propped up by assisted
reproduction, whose use is concentrated at precisely these ages and
strongly socioeconomically stratified [@beaujouan2020]: such recovery as
exists is partly medically purchased and unevenly available, not a
general loosening of the boundary. The ANR behaves like a structural
feature of modern fertility regimes, not a margin under active
renegotiation.
...
![The squeeze, 1950-2025: cross-country medians of the AAPF age (points, shaded by the rate at the peak) and the ANR (line), with interquartile ribbons. The vertical arrows mark the usable runway. Per-country trajectories: @fig-squeeze-panels.](figures/squeeze_pooled.png){#fig-squeeze}
...
and the full 45-country panel of @fig-squeeze-panels the route is
common. Country after country - from South Korea's mid-century ANR near
48 to the near-universal early peaks of south-eastern Europe - arrives
at the same configuration: a later, lower, narrower schedule pressed
against a low-forties boundary. The TFR convergence documented by
@ritchie2026convergence is the one-dimensional shadow of fertility
schedules converging on the same squeezed shape - and
@fig-squeeze-panels, which restores the full-panel presentation of the
2020 paper in distilled form, makes the correspondence direct by
ordering its panels by each country's latest period TFR
(vintage-adjusted where a series ends early; Supplement S3): descending
the ordering, from Moldova to South Korea, the trajectories do not
change shape, only how far along the shared route they have travelled. Two caveats attach: the modal age is read conservatively
(ties to the youngest age, so plateau schedules register early peaks -
Supplement S8), and single-year modal ages are noisy in small
populations, which is why the per-country panels, not the pooled
medians, are the primary output.

![The squeeze in all 45 countries, 1950-2025: the AAPF (points, shaded by the rate at the peak) and the ANR (line). Panels are ordered by latest period TFR, highest first - so the panel order itself is the TFR convergence, and the trajectories within the panels are its age-structure mechanics. For the nine series ending before 2022, the observed TFR is vintage-adjusted by the panel-median decline since the last observed year (strip labels show year: observed → adjusted); method and ordering table in Supplement S3.](figures/squeeze_trajectories.png){#fig-squeeze-panels}
...
finding about method - a blueprint rather than a personal workflow
choice: periodic, audited, AI-assisted rebuilds are a structural answer
to staleness in longitudinal quantitative knowledge work, demographic
surveillance especially, and the update cadence of a paper can become a
property of its repository rather than of its authors' spare time.
