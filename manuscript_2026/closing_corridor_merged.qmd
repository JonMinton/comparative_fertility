...
abstract: |
  ...
  43 of 45 countries' recent cohorts. Distilling each surface to three
  numbers per year shows the squeeze panel-wide: the median age of peak
  fertility rose from 24 to 31 while the ceiling recovered only to 43, the
  peak-to-ceiling distance narrowing from twenty years to twelve as the rate
  at the peak nearly halved — the age-structure mechanics beneath the widely
  noted convergence of total fertility rates. CONCLUSIONS — A decade of new data
...
popular following — carried above all by data journalism in the *Financial
Times* arguing that mass smartphone adoption tracks the decline's timing
across otherwise dissimilar countries [@burnmurdoch2026phones] — and a
growing research literature, from broadband and
...
whether the change arrived everywhere at once or diffused. The limitation is
not hypothetical: the most widely read descriptive accounts of the decline
are TFR-based — a recent Our World in Data insight documents fertility rates
converging steeply downward across countries with very different starting
points, while offering, by construction, no account of the ages through
which the convergence ran [@ritchie2026convergence]. What preserves that
structure is the fertility surface — age by cohort by rate — and a practice
of reading it that a 2020 paper by Pattaro, Vanderbloemen and Minton
developed for exactly this comparative purpose [@pattaro2020].

A reader coming to these debates fresh needs one distinction above all:
between age, period, and cohort effects. Age effects are lifecycle
regularities — fertility concentrated in the twenties and early thirties.
Period effects strike all ages in the same calendar years: a crisis, a
pandemic, a price shock. Cohort effects travel with generations, set early
and carried forward as each cohort ages. The three are easy to conflate and
expensive to confuse, and the United States supplies the cautionary example
this paper returns to repeatedly: its period TFR fell from 2.12 to 1.59
after 2007, so on period evidence alone the American fertility exception
died in the financial crisis — yet cohort-cumulative milestones show women
born as late as 1984 completing replacement-level childbearing in the
2020s. Period accounting misdates the end of the US exception by nearly two
decades, because postponed births are debited to the years in which they
did not occur rather than credited to the cohorts that eventually had them
[@sobotka2017]. Keeping the three effects distinguishable is what
Lexis-surface methods are for [@pattaro2020; @burkimsher2017]: on the
surfaces used throughout this paper, age runs up the vertical axis, cohorts
run along the horizontal, and period effects appear as diagonal bands.
...
Why did the ceiling fall, and why does it stay low? Parity control is the
canonical mechanism: stopping behaviour truncated high-parity childbearing
from the top down. But stopping plausibly interacts with a second,
intergenerational channel. As first births moved later and maternal
employment rose against scarce formal childcare, women in their forties
increasingly occupy the years the old ceiling once covered as
*grandmothers*: the late reproductive years are reallocated to the care of
daughters' children rather than to continued childbearing. Such a
reallocation is consistent with the life-history literature on the
...
fertility and fertility intentions [@aassve2012grandparenting;
@tanskanen2014grandparental]. On this reading the modern ceiling is not
merely a residue of stopping; it is actively maintained by a kin division
of labour in which a forty-something cohort adds descendants faster through
its children's fertility than through its own. The reading carries a
testable signature — ceiling recovery should be slowest where grandparental
childcare substitutes most for formal provision — which we flag for the
parity-specific extension discussed below.
...
## The squeeze: peak age against the ceiling

![The squeeze across the panel, 1950--2025: modal age of the fertility schedule (points, shaded by the ASFR at the peak — darker is higher, matching the surfaces) rising toward the effective ceiling (line).](figures/squeeze_trajectories.png){#fig-squeeze}

The surfaces contain a simpler story that can be distilled into three
numbers per country-year: the modal (peak) age of the fertility schedule,
the ASFR at that peak, and the effective ceiling. @fig-squeeze plots the
first against the third, with the second carried by shading. Pooled medians
across the panel: the peak age stood at 24 from the 1950s through the
1970s, then rose steadily — 25 (1980s), 27 (1990s), 29 (2000s), 30 (2010s),
31 (2020s) — while the ceiling descended from 45 to its 1980s–90s floor of
41 and recovered only to 43. The distance between where childbearing is
concentrated and where it effectively ends has narrowed from twenty years
to twelve — a forty percent contraction, closed mostly from below. The
shading adds a third fact: the peak is not only later but *lower*, the
median ASFR at the peak age falling by nearly half, from 0.19 (1960s) to
0.11 (2020s). Schedules have shifted later, flattened, and compressed
against a nearly fixed upper boundary.

This, we suggest, is the age-structure mechanics beneath the convergence of
period fertility documented in TFR terms elsewhere
[@ritchie2026convergence; @wilson2001; @dorius2008]. Convergence in the
total says nothing about the route; on the evidence of @fig-squeeze the
route is common. Country after country — whether starting from South
Korea's mid-century ceiling near 48 or the near-universal early peaks of
south-eastern Europe — arrives at the same configuration: a later, lower,
narrower schedule pressed against a low-forties ceiling. The corridor is
the shape into which the world's fertility schedules are converging; the
TFR convergence is its one-dimensional shadow. Two caveats attach. The
modal age is read conservatively (ties broken to the youngest age, so flat
or plateau schedules — the United States' historical plateau among them —
register early peaks), and single-year modal ages are noisy in small
populations: the per-country panels, not the pooled medians, are again the
primary output.
...
why the final departures from replacement, in country after country,
occurred pressed against a nearly fixed ceiling in the low forties, and why
fertility schedules everywhere are converging on the same later, lower,
narrower shape even as total rates converge downward
[@ritchie2026convergence]. The ceiling also
...
this paper updates and for review comments on the working papers and the
merged draft; the grandparental-care mechanism discussed in Results II was
